...
namespace {
  ...
  inline void extractOperators(const T1& couplings, T2& operators){
    ...
    DEBUG("extracting operators");
    RooAbsArg* obj = NULL;
    RooFIter itr(couplings.fwdIterator());
    while((obj = itr.next())){
      if(!obj) continue;
      RooFormulaVar* formulacoupling = dynamic_cast<RooFormulaVar*>(obj);
      RooRealVar* realcoupling = dynamic_cast<RooRealVar*>(obj);
      if(formulacoupling){
        RooArgSet* c = formulacoupling->getVariables();
        DEBUG("looking at sub-formula '"<< formulacoupling->GetName() << "' @" << formulacoupling << " with " << c->getSize() << " components @" << c);
        if(c && c->getSize() > 0){
          extractOperators(*c,operators);
        }
      } else if(realcoupling){
        operators.add(*obj);
      }
    }
  }
...
#define MAXTERMSFORMULA 100
  ...
  template<class List>
  inline FormulaList buildFormulas(const char* mfname,const RooLagrangianMorphing::ParamMap& inputParameters, const MorphFuncPattern& morphfunc, const RooArgList& couplings, const List& flags, const std::vector<List*>& nonInterfering){
  ...
  template<class T>
  inline FormulaList createFormulas(const char* name,const RooLagrangianMorphing::ParamMap& inputs, const std::vector<T*>& vertices, RooArgList& couplings, const T& flags, const std::vector<T*>& nonInterfering){
    // create the weight formulas required for the morphing
    DEBUG("building vertex map");
    VertexMap vertexmap(buildVertexMap<T>(vertices,couplings));
    ...
    FormulaList retval = buildFormulas(name,inputs,morphfuncpattern,couplings,flags,nonInterfering);
    if(retval.size() == 0){
  ...
  inline void buildSampleWeights(T1& weights, const char* fname,const RooLagrangianMorphing::ParamMap& inputParameters, FormulaList& formulas, const Matrix& inverse, T2& couplings, T3& params){
    // the two parameters "couplings" and "params" are only used in
    // some compilation schemes, so we disable warnings about them not
    // being used with this preprocessor macro
    UNUSED(couplings);
    UNUSED(params);
    int sampleidx = 0;
    //printMatrix(inverse);
    for(auto sampleit=inputParameters.begin(); sampleit!=inputParameters.end(); ++sampleit){
      ...
      for(auto formulait=formulas.begin(); formulait!=formulas.end(); ++formulait){
        ...
        RooConstVar* constVal = new RooConstVar(idx,idx,inverse(formulaidx,sampleidx));
        RooAbsReal* formula = formulait->second;
        numbers.add(*constVal);
        formulalist.add(*formula);
        formulaidx++;        
      }
      RooRealSumFunc* sampleformula = new RooRealSumFunc(name_full.Data(),name_full.Data(),numbers,formulalist);
#endif
...
#pragma GCC diagnostic pop
    ...
    //   TObject* obj;
    //   setParams(sampleit->second,params);
    //   TString fname(makeValidName(sample.c_str()));
    //   fname.Prepend("w_");
...
namespace {
  RooArgSet makeSMEFTCouplings(RooAbsCollection& operators, const char* label, const std::vector<std::string>& names) {
    ...
    for(const auto& op:names){
      DEBUG("adding "+op);
      addCoupling(couplings,TString::Format("_g%s",op.c_str()) ,TString::Format("k%s/Lambda/Lambda",op.c_str()),RooArgList(Lambda,get(operators,TString::Format("k%s",op.c_str()))),true);
    }
    return couplings;
  }
}
...
RooArgSet RooLagrangianMorphing::makeSMEFTCouplings(RooAbsCollection& operators) {
  // create the couplings needed for SMEFT
  return ::makeSMEFTCouplings(operators,"all",{"dH","eH","G","HB","HBOX","Hd","HD","He","HG","Hl1","Hl3","Hq1","Hq3","Hu","HW","HWB","ll","uG","uH","W"});
}
...
RooArgSet RooLagrangianMorphing::makeSMEFTHWWCouplings(RooAbsCollection& operators) {
  ...
  return ::makeSMEFTCouplings(operators,"HWW",{"HW"});
}
...
public:
  ...
  inline void createComponents(const RooLagrangianMorphing::ParamMap& inputParameters,const char* funcname,const std::vector<RooListProxy*>& vertices,const std::vector<RooListProxy*>& nonInterfering, const RooListProxy& flags){
    ...
    this->_formulas = ::createFormulas(funcname,inputParameters,vertices,this->_couplings,flags,nonInterfering);
  }
  ...
  inline void buildMorphingFunction(const char* name,const RooLagrangianMorphing::ParamMap& inputParameters,const std::map<std::string,int>&  storage, const RooArgList& physics,
    ...
    ::buildSampleWeights(this->_weights,name,inputParameters,this->_formulas,this->_inverse,this->_couplings,operators);
  ...
  static RooLagrangianMorphBase<Base>::CacheElem* createCache(const RooLagrangianMorphing::RooLagrangianMorphBase<Base>* func) {
    ...
    cache->createComponents(func->_paramCards,func->GetName(),func->_vertices,func->_nonInterfering,func->_flags);
  ...
  RooLagrangianMorphBase(name,title,fileName,obsName,basefolder,folders,objFilter,allowNegativeYields)
{
  ...
  for(const auto& v:vertices){
    extractOperators<T,RooArgList>(v,operators);
  }
  ...
  _curNormSet(0)
{
  ...
  for(size_t i=0; i<other._vertices.size(); ++i){
    RooListProxy* list = new RooListProxy(other._vertices[i]->GetName(),this,*(other._vertices[i]));
    this->_vertices.push_back(list);
  }
}
...
RooArgSet RooLagrangianMorphing::createWeights(const RooLagrangianMorphing::ParamMap& inputs, const std::vector<RooArgList*>& vertices, RooArgList& couplings, const RooLagrangianMorphing::FlagMap& flagValues, const RooArgList& flags, const std::vector<RooArgList*>& nonInterfering){
  ...
  FormulaList formulas = ::createFormulas("",inputs,vertices,couplings,flags,nonInterfering);
  RooArgSet operators;
  extractOperators(couplings,operators);
  ...
  Matrix inverse(::diagMatrix(size(matrix)));
  RooArgSet retval;
  ::buildSampleWeights(retval,(const char*)NULL /* name */,inputs,formulas,inverse,couplings,operators);
  return retval;
}
...
//_____________________________________________________________________________
template <class Base>
void RooLagrangianMorphing::RooLagrangianMorphBase<Base>::printSampleWeights(){
  ...
  while((w = itr.next())){
    ...
    RooAbsReal* weight = dynamic_cast<RooAbsReal*>(w);
    if(!weight) continue;
...
RooRealVar* RooLagrangianMorphing::RooLagrangianMorphBase<Base>::getParameter(const char* name) const{
  ...
  const RooArgList* args = this->getParameterSet();
  if(!args){
    return NULL;
  }
...
void RooLagrangianMorphing::RooLagrangianMorphBase<Base>::printParameters() const {
  ...
  const RooArgList* parameters = this->getParameterSet();
  RooFIter itr(parameters->fwdIterator());
  TObject* obj;
  while((obj = itr.next())){
    RooRealVar* param = dynamic_cast<RooRealVar*>(obj);
